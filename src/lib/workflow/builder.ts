...
export async function generateWorkflowFromDescription(
  ...
  // Hubspot & Salesforce patterns
  if (lowerDesc.includes('hubspot') || lowerDesc.includes('salesforce') || lowerDesc.includes('crm')) {
    const isSalesforce = lowerDesc.includes('salesforce');
    const isApproval = lowerDesc.includes('approval') || lowerDesc.includes('approve');
    ...
    return {
      name: `Add Lead to ${isSalesforce ? 'Salesforce' : 'HubSpot'}${isApproval ? ' with Approval' : ''}`,
      description: `Automatically sync new leads to CRM${isApproval ? ' after manager approval' : ''}`,
      trigger: {
        type: 'webhook',
        config: { path: 'new-lead', expectedFields: ['name', 'email'] },
      },
      actions,
      matchedTemplate: 'CRM Sync',
    };
  }

  // OpenAI patterns
  if (lowerDesc.includes('openai') || lowerDesc.includes('summarize') || lowerDesc.includes('ai')) {
    const isApproval = lowerDesc.includes('approval') || lowerDesc.includes('approve');
    const actions: any[] = [
      {
        type: 'openai',
    ...
    return {
      ...
      description: 'Process content with AI and send email',
      trigger: {
        type: 'webhook',
        config: { path: 'new-content', expectedFields: ['content'] },
      },
      actions,
      matchedTemplate: 'AI Processing',
    };
  }
