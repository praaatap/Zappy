...
export const emailHandler: ActionHandler = async (config, context) => {
  try {
    ...
    // await sendgrid.send({ to, subject, html: isHtml ? body : undefined, text: !isHtml ? body : undefined });
...
// HubSpot CRM Handler
export const hubspotHandler: ActionHandler = async (config, context) => {
  try {
    const contactEmail = replaceVariables(config.email, context.data);
    console.log('🤝 Adding to HubSpot CRM:', { contactEmail, config });
    return { success: true, output: { contactEmail, added: true } };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
};
...
// Salesforce Handler
export const salesforceHandler: ActionHandler = async (config, context) => {
  try {
    const objectType = replaceVariables(config.objectType || 'Lead', context.data);
    console.log('☁️ Adding to Salesforce:', { objectType, config });
    return { success: true, output: { objectType, createdId: 'sf-123' } };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
};

// OpenAI / AI Processing Handler
export const openaiHandler: ActionHandler = async (config, context) => {
  try {
    const prompt = replaceVariables(config.prompt, context.data);
    console.log('🧠 Processing with OpenAI:', { prompt: prompt.substring(0, 50) + '...' });
    // Simulate AI response
    return { success: true, output: { result: "Generated AI response based on prompt." } };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
};

// Wait for Approval Handler
export const waitForApprovalHandler: ActionHandler = async (config, context) => {
  try {
    const approverEmail = replaceVariables(config.approverEmail, context.data);
    console.log('⏳ Halting for human approval by:', approverEmail);
    // In a real system, you would set a state and pause. Here we'll return a special status or output.
    return { 
      success: true, 
      output: { status: 'pending_approval', approverEmail, message: 'Workflow paused, waiting for approval.' } 
    };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
...
export const actionHandlers: Record<string, ActionHandler> = {
  ...
  'wait-for-approval': waitForApprovalHandler,
};
